...
const Header: React.FC<HeaderProps> = ({ score, onLogout }) => {
  ...
  return (
    <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-12 pb-16 pt-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 bg-teal-50 px-4 py-1.5 rounded-full border border-teal-100 shadow-sm">
              ...
              <p className="text-[10px] font-black text-teal-700 uppercase tracking-[0.2em] mono">System Active</p>
            </div>
          </div>
        </div>
        <button 
      ...
      <div className="glass-premium p-10 rounded-[3.5rem] relative overflow-hidden group border-slate-200/50 min-w-[380px]">
        <div className="absolute top-0 right-0 p-8 opacity-[0.03] group-hover:opacity-[0.08] transition-opacity">
          <svg className="w-32 h-32 text-slate-900" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2L4.5 20.29L5.21 21L12 18L18.79 21L19.5 20.29L12 2Z"/></svg>
        </div>
        
        <div className="flex justify-between items-end mb-8">
          <div>
            <span className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] block mb-3">Optimization Quotient</span>
            <span className="text-7xl font-black tracking-tighter text-[#0D9488] leading-none">
              {roundedScore}<span className="text-3xl text-teal-200 font-bold ml-1">%</span>
            </span>
          </div>
          <div className="text-right">
            <span className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] block mb-3">System Tier</span>
            <span className="text-[12px] font-black uppercase tracking-widest text-white bg-slate-900 px-6 py-2.5 rounded-2xl shadow-xl">
              {roundedScore < 20 ? "INITIATE" : roundedScore < 50 ? "PROFICIENT" : roundedScore < 85 ? "MASTER" : "OPTIMIZED"}
            </span>
          </div>
        </div>
        ...
        <div className="h-4 w-full bg-slate-50 rounded-full overflow-hidden p-1 border border-slate-100 shadow-inner">
          <div 
            className="h-full bg-gradient-to-r from-teal-600 via-teal-500 to-cyan-500 rounded-full transition-all duration-1000 ease-out shadow-lg"
